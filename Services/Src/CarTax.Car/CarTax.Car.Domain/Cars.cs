...
namespace CarTax.Car.Domain
{
    public class Cars : AggregateRoot<CarId>
    {
        ...
        public static Cars Create(  byte carTypeId, CarName carName, Plack plack, bool isActive)
        {

            var car = new Cars( default );
        ...
        {
            var valid =
              CarName is not null && CarTypeId != 0 && Plack is not null &&
             (State switch
             {
               
                 CarState.Chanch => Id is not null,
                 CarState.Delete => Id is not null,
